...
const Trabalhista = () => {
  ...
  const loadSolicitacoes = async () => {
    try {
      setLoading(true);
      const response = await api.get('/trabalhista');
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400 mt-2">Gestão de demandas trabalhistas</p>
        </div>
              ...
              {loading ? (
                ...
                solicitacoes.map((solicitacao) => (
                  <tr key={solicitacao.id} className="border-b border-gray-800/50 hover:bg-red-600/5 transition-colors">
                    <td className="p-4 text-white font-medium">{solicitacao.empresa}</td>
                    <td className="p-4 text-gray-300 capitalize">{solicitacao.tipo}</td>
                    <td className="p-4 text-gray-300">{solicitacao.descricao}</td>
                    <td className="p-4 text-gray-300">{new Date(solicitacao.prazo).toLocaleDateString('pt-BR')}</td>
                    <td className="p-4">
